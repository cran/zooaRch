...
author: "Erik Otarola-Castillo, Harvard University, Jesse Wolfhagen, Stony Brook University, and Max Price, Harvard University"
date: "`r Sys.Date()`"
...
## 1. Survivorship^[The following surv.func function is only intended for traditional survivorship data. That is, the data must be assignable to discrete age classes. In zooarchaeology, this is traditionally dental eruption/wear data. Epiphyseal fusion data are treated in section 2.]

### 1.1 Introduction
The surv.func function creates confidence intervals around survivorship data. It takes bootstrap samples of the original dataset, calculates the Kaplan-Meier Estimator (KME) for each, and then calculates the confidence interval. Defaults are 1000 iterations for the bootstrap and 95% for the confidence interval. 
...
* SurviveData    The dataset described in section 1.2.
